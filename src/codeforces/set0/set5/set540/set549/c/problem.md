...
The second line contains n space-separated positive integers ai (1 ≤ ai ≤ 10^6), which represent the population of each city in Westeros.
...
### ideas
1. 最后剩余偶数的奇数（包括0），daenerys win else Stannis
2. 一共要删除 n - k 个， S 操作 （n - k + 1) / 2, D 操作 (n - k) / 2
3. 如果奇数的数量 <= (n - k) / 2, 那么肯定是D获胜，她可以在删除掉所有的奇数
4. 现在odd的数量 > (n - k) / 2
5. 假设目前奇数的数量是偶数, 且 n - k 是偶数，
6. 那么如果 S必须是第一个移除奇数的话，那么D胜利
7. 这时候D只要复制S的操作就可以了，也就是 2 * (n - k) <= 偶数的数量
8. 如果 n - k是奇数呢？那么肯定是D失败。因为S最后一步，不管前面怎么处理，S的最后一步可以改变结果
9. 