...
The first line contains three integers n, m, g (1 ≤ n ≤ 10^4, 0 ≤ m ≤ 2000, 0 ≤ g ≤ 10^4).

The second line contains n integers, each is 0 or 1, the sex of each dog, 0 represent the female and 1 represent the male.

The third line contains n integers v1, v2, ..., vn (0 ≤ vi ≤ 10^4).

Each of the next m lines describes a rich folk. On the i-th line:
- The first number is the appointed sex of i-th folk (0 or 1)
- The next two integers are wi and ki (0 ≤ wi ≤ 10^4, 1 ≤ ki ≤ 10)
- Next ki distinct integers are the indexes of appointed dogs (each index is between 1 and n)
- The last number of this line represents whether i-th folk is SmallR's good friend (0 — no or 1 — yes)
...
### ideas
1. SR可以修改狗的性别，修改每只狗的成本为v[i]
2. 然后有m个人，（他们有可能是SR的朋友或者不是）他们每个人会指定一个性别，要求将一组狗变成该性别，如果能够成功，那么给SR w[i]的钱
3. 如果不成功，如果他是SR的朋友，那么SR需要给他g的钱，否则不给钱
4. 那么对于每组狗，要么全部搞定，要么就别去搞（不管是否赔钱）
5. 