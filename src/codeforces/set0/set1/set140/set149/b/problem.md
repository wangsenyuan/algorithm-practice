...
Your task is to print the radixes of all numeral systems which can contain the time `a:b`.

## Input

The first line contains a single string as `a:b` (without the quotes). Here `a` is a non-empty string consisting of numbers and uppercase Latin letters. String `a` shows the number of hours. String `b` is a non-empty string that consists of numbers and uppercase Latin letters. String `b` shows the number of minutes. The lengths of strings `a` and `b` are from 1 to 5 characters, inclusive. Please note that strings `a` and `b` can have leading zeroes that do not influence the result in any way (for example, string `008:1` in decimal notation denotes correctly written time).
...
Print the radixes of the numeral systems that can represent the time `a:b` in increasing order. Separate the numbers with spaces or line breaks. If there is no numeral system that can represent time `a:b`, print the single integer `0`. If there are infinitely many numeral systems that can represent the time `a:b`, print the single integer `-1`.
...
Let's consider the second sample. String `2A:13` can't be perceived as correct time in any notation. For example, let's take the base-11 numeral notation. There the given string represents time 32:14 that isn't a correct time.

Let's consider the third sample. String `000B:00001` can be perceived as a correct time in an infinite number of numeral systems. If you need an example, you can take any numeral system with radix no less than 12.
