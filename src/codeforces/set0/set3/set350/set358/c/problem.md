...
The first line contains integer `n` (1 ≤ n ≤ 10⁵) — the number of Inna's commands. Then `n` lines follow, describing Inna's commands. Each line consists an integer:

- Integer `a` (1 ≤ a ≤ 10⁵) means that Inna gives Dima a command to add number `a` into one of containers.
