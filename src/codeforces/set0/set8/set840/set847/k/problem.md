...
Polycarp made n trips on buses. About each trip the stop where he started the trip and the stop where he finished are known. The trips follow in the chronological order in Polycarp's notes.
...
For example, if Polycarp made three consecutive trips: "BerBank" → "University", "University" → "BerMall", "University" → "BerBank", then he paid a + b + a = 2a + b burles. From the BerBank he arrived to the University, where he made transshipment to the other bus and departed to the BerMall. Then he walked to the University and returned to the BerBank by bus.
...
- a — the cost of a regular single trip
...
In the first example Polycarp can buy travel card for the route "BerBank" → "University" and spend 8 burles. Note that his second trip "University" → "BerMall" was made after transshipment, so for this trip Polycarp paid 3 burles. So the minimum total sum equals to 8 + 3 = 11 burles.

In the second example it doesn't make sense to buy travel cards. Note that each of Polycarp's trips (except the first) was made with transshipment. So the minimum total sum equals to 2 + 1 + 1 + 1 = 5 burles.


### ideas
1. 先算出不使用travel card情况下的cost
2. 然后开始购买travel card，节省cost
3. 