...
- **Red towers**: Each has population limit equal to 200. However, it can be built in some cell only if in that moment at least one of the neighbouring cells has a Blue Tower. Two cells are neighbours if they share a side.
...
The first line of the input contains two integers n and m (1 ≤ n, m ≤ 500). Each of the next n lines contains m characters, describing the grid. The j-th character in the i-th line is:

- `'.'` if you're allowed to build at the cell with coordinates (i, j) a tower (empty cell)
- `'#'` if there is a big hole there
