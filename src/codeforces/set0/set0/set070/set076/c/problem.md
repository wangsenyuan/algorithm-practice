...
### ideas
1. 用S表示要删除的genome的集合，那么增加的risk = sum of t(set)
2. 同时 + A(s after remove this set)
3. BACAC = (BA) + (AC) + (CA) + (AC) = 2 + 3 + 3 + 3 = 11
4. ACAC = (AC) + (CA) + (AC) = 3 + 3 + 3 = 9 + (remove B) = 9 + 1 = 10
5. BAA = (BA) + (AA) = 2 + 1 + (remove C) = 3 + 2 = 5
6. B = 0 + (remove A + C) = 0 + 6 = 6
7. AA = 1 + remove(B, C) = 1 + 3 = 4
8. 问题的关键在于使用set后，如何快速的计算出新的risk
9. 