...
Dima and Inna are using a secret code in their text messages. When Dima wants to send Inna some sentence, he writes out all words, inserting a heart before each word and after the last word. A heart is a sequence of two characters: the "less" character (`<`) and the digit three (`3`). After applying the code, a test message looks like that: `<3word1<3word2<3 ... wordn<3`.
...
- The first line contains integer `n` (1 ≤ n ≤ $10^5$) — the number of words in Dima's message
- Next `n` lines contain non-empty words, one word per line. The words only consist of small English letters
- The total length of all words doesn't exceed $10^5$
- The last line contains non-empty text message that Inna has got. The number of characters in the text message doesn't exceed $10^5$
- A text message can contain only small English letters, digits and signs more (`>`) and less (`<`)

## Output

In a single line, print `"yes"` (without the quotes), if Dima encoded the text message correctly, and `"no"` (without the quotes) otherwise.
