...
The first line contains two integers $n$ and $k$ ($1 \leq n \leq 2000$; $3 \leq k \leq 11$). The next line contains sequence of $n$ integers, each of them is either 0, or 2, or 4.

## Output

Print a single integer — the number of ways to replace zeroes by numbers 2 or 4 to get a winning sequence. As this number can be rather large, print it modulo $10^9 + 7$.
...
To better understand the game, you can see the original game on [http://gabrielecirulli.github.io/2048/](http://gabrielecirulli.github.io/2048/). Please note that the game that is described on the strip is slightly different from the original game (when the two numbers add up in the original game, they do not keep moving). Be careful, the game is addictive, there isn't much time for the contest!


### ideas
1. 假设前缀目前最后一个是pow(2, i), 那么要得到pow(2, i + 1), 那么必须要l...r达到pow(2, i), 
2. 但是，它可以往前去继续增加
3. 比如 2 8 4 0 2
4. 1000个数，到不了4096，可以到2048
5. 上面2,8,4,0,2那么8前面的2可以直接舍弃掉，它不可能再被消耗掉
6. dp[i][mask]就可以用了，表示目前，最后的几个数组成的mask的状态
7. 那么下一个数，就很好处理了
8. 