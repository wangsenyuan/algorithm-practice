# Balloon Contest Problem

## Problem Description

One tradition of ACM-ICPC contests is that a team gets a balloon for every solved problem. We assume that the submission time doesn't matter and teams are sorted only by the number of balloons they have. It means that one's place is equal to the number of teams with more balloons, increased by 1. For example, if there are seven teams with more balloons, you get the eighth place. Ties are allowed.

You should know that it's important to eat before a contest. If the number of balloons of a team is greater than the weight of this team, the team starts to float in the air together with their workstation. They eventually touch the ceiling, which is strictly forbidden by the rules. The team is then disqualified and isn't considered in the standings.
...
**Explanation:**
Limak has 20 balloons initially. There are three teams with more balloons (32, 40 and 45 balloons), so Limak has the fourth place initially. One optimal strategy is:
...
**Explanation:**
Limak has the second place and he can't improve it.
...
**Explanation:**
Limak has just enough balloons to get rid of teams 2, 3 and 5 (the teams with 81,000,000,000, 5,000,000,000 and 46,000,000,000 balloons respectively). With zero balloons left, he will get the second place (ex-aequo with team 6 and team 7).

## Solution Ideas

1. **Key Insight**: If Limak ends up with x balloons, he must have given away y = t₁ - x balloons
2. **Strategy**: The given balloons should make recipient teams float (disqualified) to be beneficial
3. **Target Teams**: Focus on teams that currently have more balloons than Limak's final count
4. **Optimization**: For each team to float, we need: t[team] + given_balloons > w[team]
5. **Greedy Approach**: 
   - Sort teams by their current balloon count in descending order
   - For each potential final balloon count x, calculate how many teams can be made to float
   - Choose the strategy that minimizes Limak's final position
6. **Cost Calculation**: To make team i float, we need to give them (w[i] + 1 - t[i]) balloons
7. **Optimal Selection**: Among teams that can be made to float, prioritize those requiring fewer balloons
8. heap 不行