...
- Each internal node $i$ ($i \leq 2^{h-1} - 1$) will have a left child with index = $2i$ and a right child with index = $2i + 1$
- The level of a node is defined as 1 for a root, or 1 + level of parent of the node otherwise. The vertices of the level h are called leaves.

The exit to the maze is located at some leaf node n, the player doesn't know where the exit is so he has to guess his way out!
...
The first line contains two integers $h$, $q$ ($1 \leq h \leq 50$, $0 \leq q \leq 10^5$), the height of the tree and the number of questions respectively.

The next $q$ lines will contain four integers each $i$, $L$, $R$, $ans$ ($1 \leq i \leq h$, $2^{i-1} \leq L \leq R \leq 2^i - 1$), representing a question as described in the statement with its answer ($ans = 1$ if the answer is "Yes" and $ans = 0$ if the answer is "No").
...
In the second sample test there are 8 leaf nodes. After the first question the exit is in the range [10, 14]. After the second and the third questions only node number 14 is correct.


### ideas 
1. i从小到达处理
2. 到第i层的时候，需要判断，这些回答是否支持，往左边、还是右边移动
3. 假设所以那些（L，R, true)的部分，都在左边，那么至少往左边移动（同时排除掉右边的部分）
4. 但是(L, R, false)的部分要怎么处理呢？
5. 这些点不断的往上移动，最终肯定是一个点，那么(L, R, true) 表示，要落在这个点上
6. (L, R, false) 表示，要避免这个点。 然后有答案的前提是，连通的点的path上，不能有避免的点
7. 如果出现了同一个点，既是green，又是red => conflict
8. 且red不能是green的父节点，同一层不能出现两个green