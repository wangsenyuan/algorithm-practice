...
Next m lines contain the descriptions of the records, one per line. The i-th line contains numbers ti, idi ($1 \leq t_i \leq 2$; $1 \leq id_i \leq 10^6$). If ti equals 1, then the corresponding record says that the car number idi arrived at the parking lot. If ti equals 2, then the corresponding record says that the car number idi departed from the parking lot.
...
### ideas
1. 需要有一个结构，可以快速的查找出，目前最宽的区间
2. 还要能很快的更新区间
3. 如果目前它是第一个区间（左边没有车）那么分配1，
4. 如果它是最后一个区间，右边没有车，分配位置n
5. 否则分配中间的位置