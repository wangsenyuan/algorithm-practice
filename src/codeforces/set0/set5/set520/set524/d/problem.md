# Problem D - Social Network Users
...
1. The number of different users online did not exceed M at any moment
2. At some second the number of distinct users online reached value M
3. The total number of users (the number of distinct identifiers) was as much as possible

Help Polycarpus cope with the test.

## Input

The first line contains three integers n, M and T (1 ≤ n, M ≤ 20,000, 1 ≤ T ≤ 86,400) — the number of queries, the record number of online users and the time when the user was online after a query was sent. 

Next n lines contain the times of the queries in the format "hh:mm:ss", where hh are hours, mm are minutes, ss are seconds. The times of the queries follow in the non-decreasing order, some of them can coincide. It is guaranteed that all the times and even all the segments of type [s, s + T - 1] are within one 24-hour range (from 00:00:00 to 23:59:59).
...
### ideas
1. 假设某个人在时刻t发送了request，然后在接下来的T个区间内，他都在线（且没有发送request）
2. 假设这样子操作，造成某个时刻超过M个人在线，需要进行调整，这时候，应该将第二个request给re-assign为第一个request的id
3. 