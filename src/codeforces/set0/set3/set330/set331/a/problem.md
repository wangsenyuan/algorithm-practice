...
1. **The first objective is to please the Beaverette**: the sum of esthetic appeal of the remaining trees must be maximum possible;
2. **The second objective is to surprise the Beaverette**: the esthetic appeal of the first and the last trees in the resulting belt must be the same;
3. **And of course, the walk should be successful**: there must be at least two trees in the woodland belt left.

Now help the Smart Beaver! Which trees does he need to cut down to win the Beaverette's heart?

## Input

- The first line contains a single integer $n$ — the initial number of trees in the woodland belt, $2 \leq n$.
- The second line contains space-separated integers $a_i$ — the esthetic appeals of each tree. All esthetic appeals do not exceed $10^9$ in their absolute value.

## Constraints

- **Subproblem A1** (30 points): $n \leq 100$
- **Subproblem A2** (70 points): $n \leq 3 \cdot 10^5$ (full problem)
...
### ideas
1. 假设 a[l] = a[r], 那么答案 = a[l] + a[r] + max(a[l+1...r-1])