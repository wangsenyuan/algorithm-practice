...
1. 首先要计算出 $\alpha$ 的上下限？
2. 但是怎么计算呢？ 一开始的下限 = 10
3. 然后要到达位置s[1], 那么 low >= s[1] * 10
4. 假设到达位置i时的，最少剩余 = w[i], 如果 (s[i+1] - s[i]) * 10 > w[i] + a
5. w[i] = 0时，可以得到a的上限（但似乎也不准确，因为没法保证w[i]等于0）
6. 是不是上下限的差 > 10 的时候，就是非unique的
7. 