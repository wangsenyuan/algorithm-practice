...
Let us remind you that number x is called a period of string s (1 ≤ x ≤ |s|), if s_i = s_{i+x} for all i from 1 to |s| - x.

## Input

The first line of the input contains three positive integers n, m and k (1 ≤ n ≤ 10^5, 1 ≤ m + k ≤ 10^5) — the length of the serial number, the number of changes made by Kefa and the number of quality checks.
...
In the second sample test three checks will be made. The first check processes substring "3493", which doesn't have period 2. Before the second check the string looks as "334334", so the answer to it is "YES". And finally, the third check processes substring "8334", which does not have period 1.


### ideas
1. 假设没有修改的情况下，如何快速的检查s是周期性的？