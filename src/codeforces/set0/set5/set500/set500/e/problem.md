...
The first line contains an integer n (2 ≤ n ≤ 2 × 10<sup>5</sup>)— the number of dominoes.

Next n lines describe the dominoes. The i-th line (1 ≤ i ≤ n) contains two space-separated integers pi, li (1 ≤ pi, li ≤ 10<sup>9</sup>)— the x-coordinate and the length of the i-th domino. It is guaranteed that p1 < p2 < ... < pn - 1 < pn.

The next line contains an integer q (1 ≤ q ≤ 2 × 10<sup>5</sup>) — the number of plans.
...
### ideas
1. 对于每个i，计算它的下一个可以推倒的位置j,
2. 对于每个query, x -> x1 -> x2 -> .... -> y
3. x可以造成x1的down，但无法造成x1+1的down，那么就需要增加 dist - len[x1]的花费
4. 这个可以先计算出来。组成一棵树，边是cost