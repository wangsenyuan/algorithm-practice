...
In the first sample, Niwel has three bears. Two bears can choose the path 1 → 2 → 4, while one bear can choose the path 1 → 3 → 4. Even though the bear that goes on the path 1 → 3 → 4 can carry one unit of weight, in the interest of fairness, he is restricted to carry 0.5 units of weight. Thus, the total weight is 1.5 units overall. Note that even though Niwel can deliver more weight with just 2 bears, he must use exactly 3 bears on this day.


### ideas
1. 因为每个bear都必须参与，且deliver的weight一样，假设为w，那么就是要让w最大
2. 假设在w的情况下，如何检查呢？
3. 那么每条边的流量上限可以调整为，可以允许的bear的数量 = int(c / w)
4. 那么此时就变成了最大流, >= x