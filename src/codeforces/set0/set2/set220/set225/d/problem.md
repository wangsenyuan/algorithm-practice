...
- Next **n** lines describe the game field. Each of these lines contains **m** characters:
  - `#` represents a wall
  - `.` is a passable square
  - `@` is an apple
  - The snake's first segment is represented by character `1`, the second one by `2`, and so on.
...
## Ideas
1. 状态搜索。怎么表示状态是个问题。
2. 蛇的长度最大是9， 在确定head的情况下， 然后下一节的位置，通过(1/0/-1,1/0/-1) (4个值，对应当前位置的4个邻居)
3. 就可以确定一个蛇的形态， ${4^9}$ * n * n 的状态。可以搞