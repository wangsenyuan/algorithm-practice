...
Initially, there are \(n\) superheroes in the avengers team having powers \(a_1, a_2, \ldots, a_n\), respectively. In one operation, they can remove one superhero from their team (if there are at least two) or they can increase the power of a superhero by \(1\). They can do at most \(m\) operations. Also, on a particular superhero at most \(k\) operations can be done.
...
## ideas
1. 假设m次操作，其中k次是减少，剩余m-k次是+1
2. 那么移除最好在一开始就完成；而且最好移除最小的前k个人
3. 剩余的 n - k 个人的平均值 = (sum(..) + m - k) / (n - k)
4. 还有一个k的条件