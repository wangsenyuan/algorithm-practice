...
### ideas
1. 考虑从1到任何位置的最短距离
2. 因为只有两行，所以要得到最短距离，不会发生往回走的情况
3. 假设从1开始，计算出了每个格子的最短距离
4. 这里对于同一列的两个格子，u, v，如果它们都可以到达
5. abs(dist(u) - dist(v)) = 1
6. 如果a在最短路径上 答案 = dist(b) - dist(a)
7. 如果a不在最短路径上, 有两可能性 dist(b) - dist(c) + 1
8. (c和a在同一列)。 或者 dist(b) - dist(c) - 1 = dist(b) - dist(a)
9. 然后只要检查a右边，是否有在最短路径上的空格子，如果有那么-2是成立的