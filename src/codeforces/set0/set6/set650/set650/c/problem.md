...
The first line of the input contains two integers n and m (1 ≤ n, m ≤ 1000), the number of rows and the number of columns of the table respectively.

Each of the following n rows contain m integers ai, j (1 ≤ ai, j ≤ 10^9) that are the values in the table.
...
In the first sample test, despite the fact a1, 2 ≠ a2, 1, they are not located in the same row or column so they may become equal after the compression.