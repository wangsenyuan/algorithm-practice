...
Then n lines follow, i-th line contains the pair of integers ai, bi (0 ≤ ai, bi ≤ 10^6), where ai is the ID number of a person in front of a student and bi is the ID number of a person behind a student. The lines are given in the arbitrary order. Value 0 is given instead of a neighbor's ID number if the neighbor doesn't exist.

The ID numbers of all students are distinct. It is guaranteed that the records correspond to the queue where all the students stand in some order.
...
### ideas
1. 要给i分配正确的id，以满足这个队列的要去
2. 从特殊的开始，假设有一个它的pre = 0, 那么它就是第一个（它的序号知道了,i,但是它的id不确定)
3. 那么x就是第二个
4. 先放偶数位，然后从后往前放奇数位