...
More formally, let the secret string be s and the second player is trying to guess it with a string x. The number of "bulls" is a number of such positions i ($1 \leq i \leq 4$) where $s[i] = x[i]$. The number of "cows" is a number of such digits c that s contains c in the position i (i.e. $s[i] = c$), x contains c, but $x[i] \neq c$.
...
### ideas
1. 使用0000, 去查，如果是(0, 0), 说明0不会出现
2. 如果是(1, 0), 说明0肯定出现了() 这样子的数字有4个
3. 假设是 abcd (4个数字， 4 * 3 * 2 = 24种可能性)
4.  可以在9次里面找出这4个数字，那么还剩下 41次，查询。
5.  那就好搞了