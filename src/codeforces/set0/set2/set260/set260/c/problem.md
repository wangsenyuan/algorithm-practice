# Problem C: Little Vasya and Balls
...
### ideas
1. 只操作了一次，最后一个球落入了位置x
2. 这里的x表示的是，最后一个球被放入的位置
3. 是不是最靠近x的右边，的最小的那个位置，是开始位置？
4. 假设这个位置是y, a[y-1] >= a[y] < a[y+1] < a[y+2] ... a[x]  y有可能等于x
5. 有没有可能在y的后面？不可能，因为从y里面取出后，能够放回y的，肯定是最少的，它后面的，只会比它多，直到x（x后面获取的的比x少1）
6. 有没有可能a[y-1] 也不可能