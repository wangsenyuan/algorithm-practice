...
- For each non-border square of the pipe there are exactly two side-adjacent squares that also belong to the pipe
- For each border square of the pipe there is exactly one side-adjacent cell that also belongs to the pipe

## Allowed Piping Routes
...
- If the square of the map is marked by character '.', then the square is empty and the pipe can go through it.
- If the square of the map is marked by character '#', then the square is full and the pipe can't go through it.
