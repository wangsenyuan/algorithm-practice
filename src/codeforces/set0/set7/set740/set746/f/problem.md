...
The first line contains three integers n, w and k (1 ≤ w ≤ n ≤ 2·10⁵, 1 ≤ k ≤ 2·10⁹) — the number of songs in the playlist, the number of songs Sasha can listen to partly and time in minutes which Sasha needs to reach work.

The second line contains n positive integers a₁, a₂, ..., aₙ (1 ≤ ai ≤ 10⁴), where ai equals the pleasure Sasha gets after listening to the i-th song.

The third line contains n positive integers t₁, t₂, ..., tₙ (2 ≤ ti ≤ 10⁴), where ti equals the length of the i-th song in minutes.
...
### ideas
1. 假设从l开始听, 一直到r结束
2. 那么从中要选择w首歌，只听一半，那么这w个应该是最长的w首歌（和它的a[?]无关）
3. 双指针，还要一个能够维护前w大的数据结构（进入和离开）