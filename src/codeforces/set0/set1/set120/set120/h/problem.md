...
1. it occurs in s as a subsequence,
2. its length ranges from one to four characters.
...
### ideas
1. 对于原来的words，将它们变形后，得到同样大小的一个集合，集中中每个元素不超过长度4
2. 按照长度从短到长处理，且尽量的使用短的（这样给后面的留下空间）