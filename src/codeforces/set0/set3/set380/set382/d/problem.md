# Problem D: Chessboard Game

## Problem Statement

Ksenia has a chessboard of size $n \times m$. Each cell of the chessboard contains one of the characters: `"<"`, `">"`, `"^"`, `"v"`, `"#"`. The cells that contain character `"#"` are blocked. We know that all chessboard cells that touch the border are blocked.
...
The first line contains two integers, $n$ and $m$ ($1 \leq n, m \leq 2000$) — the sizes of the board. Each of the following $n$ lines contains $m$ characters – the board's description. Each character is one of the characters: `"<"`, `">"`, `"^"`, `"v"`, `"#"`.
...
### ideas
1. 如果存在一个环 => -1 (可以得♾️分)
2. 在不存在环的情况下，就是最远路径。但是两个pawn不能从同一个最远路径开始
3. 