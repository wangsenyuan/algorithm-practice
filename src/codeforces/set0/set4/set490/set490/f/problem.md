...
The first line of the input contains integer `n` (`2 ≤ n ≤ 6000`) — the number of cities in Treeland. The next line contains `n` integers `r1, r2, ..., rn` (`1 ≤ ri ≤ 10^6`), where `ri` is the population of the `i`-th city. The next `n - 1` lines contain the descriptions of the roads, one road per line. Each road is defined by a pair of integers `aj, bj` (`1 ≤ aj, bj ≤ n`) — the pair of the numbers of the cities that are connected by the `j`-th road. All numbers in the lines are separated by spaces.
...
### ideas
1. 最多6000个不同的数，压缩一下
2. dp[u][i] 表示如果在子树u中，最大的数是i时的从leaf到u的最长path
3. fp[u][i]表示从u到leaf，i是最小值时的最长path
4. dp[u][i] = max(dp[v][i], dp[v][j] + 1 if r[u] = i)
5. fp[u][i] = max(fp[v][i], fp[v][j] + 1 if r[u] = i)
6. 还需要re-root一下? 貌似不需要