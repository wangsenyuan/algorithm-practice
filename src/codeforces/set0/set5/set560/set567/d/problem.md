...
The third line contains `m` distinct integers x₁, x₂, ..., xₘ, where xᵢ is the number of the cell where Bob made the i-th shot. The cells are numbered from left to right from 1 to `n`.
...
### ideas
1. 就是找到某个位置，alice说miss的时候，无法在位置x[i]的前面，或者后面放置一艘完整的ship时，就是在说谎
2. 就是空的位置，要能放置k个ship