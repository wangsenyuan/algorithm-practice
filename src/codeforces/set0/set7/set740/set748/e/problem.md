...
The second line consists of n positive integers a1, a2, ..., an (1 ≤ ai ≤ 10^7), where ai stands for the number of slices the i-th tangerine consists of.
...
**Explanation:** In the first example Santa should divide the second tangerine into two parts with 5 and 4 slices. After that he can present the part with 5 slices to the first pupil and the whole first tangerine (with 5 slices, too) to the second pupil.
...
**Explanation:** In the second example Santa should divide both tangerines, so that he'll be able to present two parts with 6 slices and two parts with 7 slices.
...
**Explanation:** In the third example Santa Claus can't present 2 slices to 3 pupils in such a way that everyone will have anything.


### ideas
1. 感觉可以二分，假设每个小孩至少得到 x个
2. 然后从高到低分配，可以将a[i] 分配给 y = a[i] / x 个小孩
3. 那么sum(y) >= k?
4. 会不会太简单了？